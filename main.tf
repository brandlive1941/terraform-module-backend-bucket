...
resource "google_compute_backend_bucket" "site" {
  ...
  custom_response_headers = ["Content-Security-Policy: default-src 'self' https:; style-src 'self' 'unsafe-inline'; script-src 'self' 'unsafe-inline' 'unsafe-eval'; img-src 'self' 'https://greenroom-assets-${var.environment}.greenroom-${var.environment}.com' data:; font-src 'self'; connect-src 'self'; frame-src 'self';"]
  enable_cdn  = true
}
